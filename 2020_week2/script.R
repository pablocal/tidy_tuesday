...
df %>% 
  filter(strength < 11) %>% 
  group_by(category) %>% 
  summarise(stre_mean = mean(strength),
            stre_max = max(strength),
            stre_min = min(strength))
...
df_plot <- df %>% 
  ...
  ggplot(df_plot) +
  geom_violin(aes(reorder(category, -stre_mean), strength), fill = "white", col = "darkgreen", alpha = .8) +
  geom_point(aes(reorder(category, -stre_mean), y = stre_mean), col = "darkgreen", shape = 19, size = 3) +
  geom_text(data = pass, aes(x = category, y = stre, label = pass), col = "orange", family = gfont, size = 3.5) +
  geom_curve(x = 1,  xend = 1.2, y = 8, yend = 9, col = "darkgreen", arrow = arrow(angle = 20, ends = "first", type = "closed", length = unit(0.25, "cm")), curvature = -.35, size = .2) +
  scale_y_continuous(limits = c(-1, 11),
                     breaks = seq(0, 10, 2)) +
  scale_x_discrete(position = "top") +
  annotate(geom = "text", x = 1.5, y = 9, label = "Average\nstrength", family = gfont, color = "darkgreen", size = 2.75, fontface = "bold", lineheight = .8) +
  labs(title = "A typology of 'bad' passwords",
       ...
       caption = "@pablocalv for #TidyTuesday · 2020 W2") +
